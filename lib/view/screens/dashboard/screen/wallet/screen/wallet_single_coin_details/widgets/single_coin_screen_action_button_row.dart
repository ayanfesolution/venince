...
class SingleCoinScreenButtonRow extends StatelessWidget {
  final SingleCoinWalletDetailsController controller;
  final String walletType;
  const SingleCoinScreenButtonRow({super.key, required this.controller, required this.walletType});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          if (walletType == 'spot' ? controller.walletRepository.apiClient.getWalletTypes()?.spot?.configuration?.deposit?.status == '1' : controller.walletRepository.apiClient.getWalletTypes()?.funding?.configuration?.deposit?.status == '1') ...[
            Expanded(
              child: ElevatedButton(
                ...
                onPressed: () {
                  Get.toNamed(RouteHelper.depositScreen, arguments: [walletType, "${controller.singleWalletDetailsModelData.data?.currency?.id}"]);
                },
                child: Row(
          ...
          if (walletType == 'spot' ? controller.walletRepository.apiClient.getWalletTypes()?.spot?.configuration?.withdraw?.status == '1' : controller.walletRepository.apiClient.getWalletTypes()?.funding?.configuration?.withdraw?.status == '1') ...[
            Expanded(
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(
                  elevation: 0,
                ...
                onPressed: () {
                  Get.toNamed(RouteHelper.withdrawScreen, arguments: [walletType, controller.singleWalletDetailsModelData.data?.currency?.id]);
                },
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  crossAxisAlignment: CrossAxisAlignment.center,
          ...
          if (walletType == 'spot'
            ...
            Expanded(
              child: ElevatedButton(
                ...
                onPressed: () {
                  Get.toNamed(RouteHelper.transferScreen, arguments: [walletType, controller.singleWalletDetailsModelData.data?.currency?.id]);
                },
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  crossAxisAlignment: CrossAxisAlignment.center,
